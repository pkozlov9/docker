...
class XHProfRuns_Default implements iXHProfRuns {
  ...
  function list_runs() {
    if (is_dir($this->dir)) {
        echo '</br></br>Current profiling state: ' .
            (XHPROF && XHPROF_ENABLED ? '<a href="?action=profiling_off">On</a>' : '<a href="?action=profiling_on">Off</a>');
        echo '</br></br><a href="?action=clear_all">Clear all runs</a>';

        if(isset($_GET['action'])) {
            switch ($_GET['action']) {
                case 'clear_all':
                    shell_exec('rm -rf ' . __DIR__ . '/../../xhprof_runs/*');
                    break;
                case 'profiling_on':
                    $file = file_get_contents(XHPROF_CONSTANTS);
                    $file = str_replace("define('XHPROF_ENABLED', false);", "define('XHPROF_ENABLED', true);", $file);
                    file_put_contents(XHPROF_CONSTANTS, $file);
                    break;
                case 'profiling_off':
                    $file = file_get_contents(XHPROF_CONSTANTS);
                    $file = str_replace("define('XHPROF_ENABLED', true);", "define('XHPROF_ENABLED', false);", $file);
                    file_put_contents(XHPROF_CONSTANTS, $file);
                    break;
                default:
                    break;
            }
            echo '<script>window.location.replace("/xhprof/")</script>'; exit;
        }

        $files = glob("{$this->dir}/*.{$this->suffix}");
        echo "<hr/>Existing runs(" . count($files) . "):\n<ul>\n";
        ...
        foreach ($files as $file) {
            ...
            if ($run) {
                ...
                if (count($runs_array) == 1) {
                    $xhprof_data = $xhprof_runs_impl->get_run($runs_array[0], $source, $desc);
                }
            }
            ...
            echo '<li>' .
                ...
                "<small> " . date("Y-m-d H:i:s", filemtime($file)) . "</small></br>" .
                "<small>" .  $xhprof_data['QUERY']['REQUEST_URI'] . "</small>" .
                (count($xhprof_data['QUERY']['GET']) > 0 ?
                    '<small><pre>GET ' .  print_r($xhprof_data['QUERY']['GET'], true) . '</small>' : '') .
                (count($xhprof_data['QUERY']['POST']) > 0 ?
                    '<small><pre>POST ' .  print_r($xhprof_data['QUERY']['POST'], true) . '</small>' : '');
                "</li>";
        }
        echo "</ul>\n";
    }
