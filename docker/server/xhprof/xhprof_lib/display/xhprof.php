...
function profiler_report ($url_params,
  ...
  $query =      '  <dt>Run 1 Query</dt>' .
                '  <dd>' . $run1_data['QUERY']['REQUEST_URI'] . '  </dd>' .
                (count($run1_data['QUERY']['GET']) > 0 ?
                    '  <dd><pre>GET ' . print_r($run1_data['QUERY']['GET'], true) . '</pre>  </dd>' : '') .
                (count($run1_data['QUERY']['POST']) > 0 ?
                    '  <dd><pre>POST ' . print_r($run1_data['QUERY']['POST'], true) . '</pre>  </dd>' : '');
  if(!empty($run2_data)){
      $query .= '  <dt>Run 2 Query</dt>' .
                '  <dd>' . $run2_data['QUERY']['REQUEST_URI'] . '  </dd>' .
                (count($run2_data['QUERY']['GET']) > 0 ?
                '  <dd><pre>GET ' . print_r($run2_data['GET']['POST'], true) . '</pre>  </dd>' : '') .
                (count($run2_data['QUERY']['POST']) > 0 ?
                '  <dd><pre>POST ' . print_r($run2_data['QUERY']['POST'], true) . '</pre>  </dd>' : '');
  }

  unset($run1_data['QUERY']);
  unset($run2_data['QUERY']);
  ...
  $links = array();
  $links [] =  '<a href="http://' . $_SERVER['HTTP_HOST'] . '/xhprof/xhprof_html/index.php">List Of Runs</a>';
